...
function buildTreemapData(data) {
  // Try to find the best matching column names for company and position
  const companyCol = Object.keys(data[0]).find(k => /company/i.test(k));
  const positionCol = Object.keys(data[0]).find(k => /position|title/i.test(k));
  ...
  data.forEach(row => {
    const company = row[companyCol] && row[companyCol].trim() !== '' ? row[companyCol] : 'Unknown Company';
    const position = row[positionCol] && row[positionCol].trim() !== '' ? row[positionCol] : 'Unknown Position';
